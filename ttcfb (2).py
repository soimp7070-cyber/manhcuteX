import requests, base64, uuid, os, json, threading, random
...
# ANIMATION EFFECTS
def neon_text(text, color1, color2):
    return f"{color1}{Style.BRIGHT}{text}{Style.RESET_ALL}"
...
def loading_animation(duration=2):
    chars = ["◐", "◓", "◑", "◒"]
    colors = [neon_pink, neon_blue, neon_green, neon_yellow]
    end_time = time.time() + duration
    while time.time() < end_time:
        for i, char in enumerate(chars):
            sys.stdout.write(f"\r{colors[i]}{Style.BRIGHT}{char} Loading...{Style.RESET_ALL}")
            sys.stdout.flush()
            sleep(0.1)
...
def banner():
    ...
    # NEON BANNER WITH GRADIENT EFFECT
    title_lines = [
        "████████╗██████╗░███████╗███████╗██╗░░░░░██╗░░░██╗",
        "╚══██╔══╝██╔══██╗██╔════╝██╔════╝██║░░░░░██║░░░██║",
        "░░░██║░░░██║░░██║█████╗░░█████╗░░██║░░░░░╚██╗░██╔╝",
    ...
    subtitle = "🔥 ⸜(｡˃ ᵕ ˂ )⸝էօօӀ.էէç.ƒąçҽҍօօҟ♡🔥"
    author = "⚡ DEVELOPED By Khangcoder ⚡"
   

    # Print with neon effects
    print()
    for line in title_lines:
        gradient_colors = [neon_pink, neon_blue, neon_cyan, neon_green, neon_yellow, neon_purple]
        print(gradient_text(line, gradient_colors))
        sleep(0.05)
    
    print(f"\n{neon_yellow}{Style.BRIGHT}{subtitle.center(50)}{Style.RESET_ALL}")
    print(f"{neon_cyan}{Style.BRIGHT}{author.center(50)}{Style.RESET_ALL}")
    
    # Decorative line
    print(f"\n{neon_pink}{Style.BRIGHT}" + "═" * 60 + f"{Style.RESET_ALL}")
    print(f"{neon_green}{Style.BRIGHT}⚡ TTCFB • BY Khangcoder • SIU VÍP ⚡{Style.RESET_ALL}")
    print(f"{neon_pink}{Style.BRIGHT}" + "═" * 60 + f"{Style.RESET_ALL}\n")    
...
def _delay(value):
    neon_colors = [neon_pink, neon_blue, neon_green, neon_yellow, neon_cyan]
    counter = 0
    while not(value <= 1):
        value -= 0.123
        color = neon_colors[counter % len(neon_colors)]
        counter += 1
        display_value = str(value)[0:5]
        
        # Enhanced neon delay display
        status_text = f"{neon_purple}⚡ {neon_blue}NEON-DELAY {neon_green}» {neon_yellow}{display_value}s"
        print(f"{color}{Style.BRIGHT}{status_text}{Style.RESET_ALL}", ' ' * 20, end='\r')
        sleep(0.02)
        
        # Flash effect
        print(f"{neon_white}{Style.BRIGHT}{status_text}{Style.RESET_ALL}", ' ' * 20, end='\r')
        sleep(0.01)

def countdown(value):
    neon_colors = [neon_red, neon_orange, neon_yellow, neon_green, neon_blue]
    counter = 0
    while not(value <= 1):
        value -= 0.123
        color = neon_colors[counter % len(neon_colors)]
        ...
        # Enhanced countdown with neon effects
        countdown_text = f"{neon_red}⏱️ {neon_orange}NEON-COUNTDOWN {neon_yellow}» {neon_green}{display_value}s"
        print(f"{color}{Style.BRIGHT}{countdown_text}{Style.RESET_ALL}", ' ' * 20, end='\r')
        sleep(0.02)
        
        # Pulsing effect
        print(f"{neon_white}{Style.BRIGHT}{countdown_text}{Style.RESET_ALL}", ' ' * 20, end='\r')
        sleep(0.01)

def chongblock(delaybl):
    neon_colors = [neon_purple, neon_pink, neon_blue, neon_cyan]
    for i in range(delaybl, -1, -1):
        color = neon_colors[i % len(neon_colors)]
        block_text = f"🛡️ {neon_red}BLOCK PROTECTION {neon_yellow}» {neon_green}{i}s remaining"
        print(f"{color}{Style.BRIGHT}{block_text}{Style.RESET_ALL}", end='\r')
        sleep(1)
        # Clear line
        print(' ' * 60, end='\r')
...
def Nhap_Cookie():
    ...
    # Neon header for cookie input
    print(f"{neon_pink}{Style.BRIGHT}" + "═" * 60 + f"{Style.RESET_ALL}")
    print(f"{neon_cyan}{Style.BRIGHT}🔐 QUẢN LÝ COOKIE FACEBOOK 🔐{Style.RESET_ALL}")
    print(f"{neon_pink}{Style.BRIGHT}" + "═" * 60 + f"{Style.RESET_ALL}")
    
    while True:
        demck += 1
        prompt_text = f"{neon_yellow}#{neon_green}{demck}{neon_yellow} » {neon_blue}Nhập Cookie Facebook"
        ck = input(f"{neon_purple}{Style.BRIGHT}{prompt_text}: {neon_white}")
        
        if ck == '' and demck > 1:
            break
            
        print(f"{neon_orange}{Style.BRIGHT}🔍 Đang xác minh cookie...{Style.RESET_ALL}")
        _info = _Infofb(ck)
        
        if _info == False:
            print(f"{neon_red}{Style.BRIGHT}❌ COOKIE KHÔNG HỢP LỆ! Vui lòng thử lại.{Style.RESET_ALL}")
            demck -= 1
        else:
            # Success message with neon effects
            print(f"{neon_green}{Style.BRIGHT}" + "✓" * 50 + f"{Style.RESET_ALL}")
            print(f"{neon_blue}🆔 UID: {neon_yellow}{_info[1]} {neon_pink}| {neon_blue}👤 NAME: {neon_cyan}{_info[2]}{Style.RESET_ALL}")
            print(f"{neon_green}{Style.BRIGHT}" + "✓" * 50 + f"{Style.RESET_ALL}")
            listck.append(ck)
            
            # Continue prompt
            continue_choice = input(f"{neon_green}Thêm cookie khác? (Enter để tiếp tục, phím bất kỳ để kết thúc): {neon_white}")
            if continue_choice.strip() != '':
                break
...
def Main():
    ...
    # Welcome animation
    welcome_msgs = [
        f"{neon_pink}CHÀO MỪNG ĐẾN VỚI CÔNG CỤ TTCFB!",
        f"{neon_blue}TỰ ĐỘNG HOÁ FACEBOOK qua TTC", 
        f"{neon_green}TOOL ĐÃ KÍCH HOẠT",
        f"{neon_yellow}CHẾ ĐỘ HIỆU SUẤT CAO ĐANG CHẠY"
    ]
    ...
    print(f"\n{neon_cyan}{Style.BRIGHT}" + "⚡" * 30 + f"{Style.RESET_ALL}\n")
    ...
    while True:
        if os.path.exists('configttc.txt'):
            ...
            if login.json()['status'] == 'success':
                ...
                # Neon menu
                print(f"{neon_pink}{Style.BRIGHT}" + "═" * 50 + f"{Style.RESET_ALL}")
                print(f"{neon_cyan}⚡ TTC ACCOUNT LOGIN SUCCESS ⚡{Style.RESET_ALL}")
                print(f"{neon_pink}{Style.BRIGHT}" + "═" * 50 + f"{Style.RESET_ALL}")
                
                print(f'{neon_yellow}[{neon_white}1{neon_yellow}] {neon_green}Đăng nhập tài khoản {neon_blue}{user}')
                print(f'{neon_yellow}[{neon_white}2{neon_yellow}] {neon_red}Nhập Access_Token TTC mới')
                
                chon = input(f'{neon_yellow}Lựa chọn của bạn {neon_pink}[1/2]: {neon_white}')
                if chon == '2':
                    os.remove('configttc.txt')
                elif chon == '1':
                    pass
                else:
                    print(f'{neon_red}{Style.BRIGHT}Lựa chọn không hợp lệ!{Style.RESET_ALL}')
                    continue
            else:
                os.remove('configttc.txt')
        if not os.path.exists('configttc.txt'):
            print(f"{neon_pink}{Style.BRIGHT}" + "═" * 50 + f"{Style.RESET_ALL}")
            print(f"{neon_yellow}🔑 CẦN NHẬP TTC ACCESS TOKEN{Style.RESET_ALL}")
            print(f"{neon_pink}{Style.BRIGHT}" + "═" * 50 + f"{Style.RESET_ALL}")
            tk = input(f'{neon_cyan}Nhập Access_token TTC: {neon_white}')
            with open('configttc.txt', 'w') as f:
                f.write(tk)
            print(f"{neon_green}{Style.BRIGHT}✅ Đã lưu token thành công!{Style.RESET_ALL}")
        with open('configttc.txt', 'r') as f:
            tk = f.read()
        login = requests.post('https://tuongtaccheo.com/logintoken.php',data={'access_token': tk})
        if login.json()['status'] == 'success':
            ...
            # Neon account info display
            print(f"{neon_purple}{Style.BRIGHT}" + "═" * 60 + f"{Style.RESET_ALL}")
            print(f"{neon_blue}👤 Account: {neon_yellow}{user} {neon_pink}| {neon_green}💰 Balance: {neon_cyan}{sodu} Xu{Style.RESET_ALL}")
            print(f"{neon_purple}{Style.BRIGHT}" + "═" * 60 + f"{Style.RESET_ALL}")
            
            # Success animation
            print(f"{neon_green}{Style.BRIGHT}✅ LOGIN SUCCESSFUL!{Style.RESET_ALL}")
            sleep(1)
            break
        else:
            os.remove ('configttc.txt')
    ...
    while True:
        if os.path.exists('Cookie_FB.txt'):
            # Neon cookie menu
            print(f"{neon_pink}{Style.BRIGHT}" + "═" * 60 + f"{Style.RESET_ALL}")
            print(f"{neon_cyan}🍪 TUỲ CHỌN COOKIE FACEBOOK 🍪{Style.RESET_ALL}")
            print(f"{neon_pink}{Style.BRIGHT}" + "═" * 60 + f"{Style.RESET_ALL}")
            
            print(f'{neon_yellow}[{neon_white}1{neon_yellow}] {neon_green}Sử dụng Cookie Facebook đã lưu')
            print(f'{neon_yellow}[{neon_white}2{neon_yellow}] {neon_red}Nhập Cookie Facebook mới')
            
            chon = input(f'{neon_yellow}Lựa chọn của bạn {neon_pink}[1/2]: {neon_white}')
            if chon == '1':
                print(f'{neon_blue}{Style.BRIGHT}🔄 Đang tải cookie đã lưu...{Style.RESET_ALL}')
                loading_animation(1)
                with open('Cookie_FB.txt', 'r') as f:
                    listck = json.loads(f.read())
                    print(f"{neon_green}{Style.BRIGHT}✅ Đã tải {len(listck)} cookie!{Style.RESET_ALL}")
                    break
            elif chon == '2':
                print(f"{neon_red}{Style.BRIGHT}" + "⚠" * 50 + f"{Style.RESET_ALL}")
                print(f"{neon_orange}Đang xoá cookie đã lưu...{Style.RESET_ALL}")
    ...
    banner()
    # Neon account info
    print(f"{neon_purple}{Style.BRIGHT}" + "═" * 70 + f"{Style.RESET_ALL}")
    print(f"{neon_blue}👤 User: {neon_yellow}{user} {neon_pink}| {neon_green}💰 Balance: {neon_cyan}{sodu} Xu{Style.RESET_ALL}")
    print(f"{neon_purple}{Style.BRIGHT}" + "═" * 70 + f"{Style.RESET_ALL}")
    Write.Print('''[1] - LIKEVIP\n[2] - LIKETHUONG\n[3] - REACTIONVIP\n[4] - REACTION\n[5] - REACTIONCMT\n[6] - CMTCHEO\n[7] - SHARE\n[8] - LIKEPAGE\n[9] - FOLLOW\n🌟 Lưu Ý: Có Thể Chọn Nhiều Nhiệm Vụ (VD: 123...)\n''',Colors.rainbow,interval=0.0001)
    print(f"{red}════════════════════════════════════════════════════")
    nhiemvu = input(f'{neon_cyan}Nhập nhiệm vụ cần chạy {neon_pink}(VD: 123): {neon_white}')
    # Neon configuration panel
    print(f"\n{neon_green}{Style.BRIGHT}" + "⚙" * 35 + f" BẢNG CẤU HÌNH " + "⚙" * 35 + f"{Style.RESET_ALL}")
    
    min = int(input(f'{neon_cyan}Delay tối thiểu (giây): {neon_white}'))
    max = int(input(f'{neon_cyan}Delay tối đa (giây): {neon_white}'))
    nvblock = int(input(f'{neon_purple}Chống block sau bao nhiêu nhiệm vụ: {neon_white}'))
    delaybl = int(input(f'{neon_blue}Nghỉ ngơi sau {neon_yellow}{nvblock}{neon_blue} nhiệm vụ (giây): {neon_white}'))
    doinick = int(input(f'{neon_green}Đổi nick sau bao nhiêu nhiệm vụ: {neon_white}'))
    nhiemvuloi = int(input(f'{neon_red}Xoá cookie sau bao nhiêu nhiệm vụ lỗi: {neon_white}'))
    
    # Confirmation
    print(f"\n{neon_yellow}{Style.BRIGHT}📋 TÓM TẮT CẤU HÌNH:{Style.RESET_ALL}")
    print(f"{neon_pink}Delay: {neon_white}{min}-{max}s {neon_pink}| Chống Block: {neon_white}mỗi {nvblock} nhiệm vụ")
    print(f"{neon_pink}Nghỉ Ngơi: {neon_white}{delaybl}s {neon_pink}| Đổi Nick: {neon_white}mỗi {doinick} nhiệm vụ")
    print(f"{neon_pink}Giới Hạn Lỗi: {neon_white}{nhiemvuloi} nhiệm vụ lỗi{Style.RESET_ALL}")
    print(f"{neon_green}{Style.BRIGHT}✅ Đã lưu cấu hình! Bắt đầu tự động hoá...{Style.RESET_ALL}")
    while True:
        if len(listck) == 0:
            print(f'{neon_red}{Style.BRIGHT}💥 All cookies deleted! Please enter new cookies.{Style.RESET_ALL}')
            listck = Nhap_Cookie()
            with open('Cookie_FB.txt', 'w') as f:
                json.dump(listck, f)
        for ck in listck:
            ...
            if _info != False:
                ...
                print(f'{neon_red}{Style.BRIGHT}💀 Cookie Account {neon_yellow}{uidfb} {neon_red}DIED!{Style.RESET_ALL}', end='\r')
                sleep(3)
                print(' ' * 50, end='\r')
                listck.remove(ck)
                continue
            ...
            if '1' in cauhinh:
                ...
                print(f"{neon_green}{Style.BRIGHT}" + "⚡" * 60 + f"{Style.RESET_ALL}")
                print(f"{neon_blue}🚀 Running UID: {neon_yellow}{uidfb} {neon_pink}| {neon_cyan}FB: {neon_white}{tenfb}{Style.RESET_ALL}")
                print(f"{neon_green}{Style.BRIGHT}" + "⚡" * 60 + f"{Style.RESET_ALL}")
            else:
                # Neon error message
                print(f"{neon_red}{Style.BRIGHT}" + "❌" * 60 + f"{Style.RESET_ALL}")
                print(f"{neon_orange}⚠️ Config Failed UID: {neon_yellow}{uidfb} {neon_pink}| {neon_red}FB: {neon_white}{tenfb}{Style.RESET_ALL}")
                print(f"{neon_red}{Style.BRIGHT}" + "❌" * 60 + f"{Style.RESET_ALL}")
                continue
            ptool = 0
            while True:
                        ...
                        for x in like2:
                            ...
                            if like == False:
                                ...
                                if 'mess' in nhan:
                                    ...
                                    # Neon success message for LIKE VIP
                                    success_msg = f"{neon_pink}[{neon_white}{dem}{neon_pink}] {neon_cyan}[{neon_yellow}{time}{neon_cyan}] {neon_green}[LIKE VIP] {neon_blue}[DONE] {neon_purple}[HIDDEN] {neon_yellow}[+1100] {neon_cyan}[{xu} Xu]"
                                    print(f"{Style.BRIGHT}{success_msg}{Style.RESET_ALL}")
                                    loilike = 0
                                    if dem % doinick == 0:
                                        ptool = 1
                                        break
                        ...
                        for x in like2:
                            ...
                            if like == False:
                                ...
                                if 'mess' in nhan:
                                    ...
                                    dem+=1
                                    time = datetime.now().strftime("%H:%M:%S")
                                    # Neon success message for LIKE NORMAL
                                    success_msg = f"{neon_pink}[{neon_white}{dem}{neon_pink}] {neon_cyan}[{neon_yellow}{time}{neon_cyan}] {neon_green}[LIKE NORMAL] {neon_blue}[DONE] {neon_purple}[HIDDEN] {neon_yellow}[+300] {neon_cyan}[{xu} Xu]"
                                    print(f"{Style.BRIGHT}{success_msg}{Style.RESET_ALL}")                       
                                    loilike = 0
                                    if dem % doinick == 0:
                                        ptool = 1
