...
# --- CẤU HÌNH MÀU SẮC ---
BLUE      = "\033[1;34m"    # Xanh dương đậm
CYAN      = "\033[1;36m"    # Xanh lơ
RED       = "\033[1;31m"    # Đỏ
GREEN     = "\033[1;32m"    # Xanh lá
YELLOW    = "\033[1;33m"    # Vàng
WHITE     = "\033[1;37m"    # Trắng
PURPLE    = "\033[1;35m"    # Tím
PINK      = "\033[1;95m"    # Hồng đậm
LIGHT_PINK= "\033[95m"      # Hồng nhạt
RESET     = "\033[0m"       # Reset màu

# Gán lại biến cho code cũ
do    = RED      # Lỗi
luc   = GREEN    # Thành công
vang  = YELLOW   # Số
trang = WHITE    # Chữ
xanh  = BLUE     # Chủ đạo
cyan  = CYAN     # Điểm nhấn
tim   = PURPLE   # Tím
hong  = PINK     # Hồng
hong_nhat = LIGHT_PINK # Hồng nhạt

# Ký tự nhập liệu đồng bộ
thanh = f'{xanh}[{cyan}►{xanh}] {trang}=> '

listCookie = []
nhiem_vu_da_bat = {}

# DANH SÁCH TỬ VIỆT NAM (Offline Mode)
NAM = ["Nguyễn Văn An", "Trần Văn Bình", "Lê Văn Cường", "Phạm Văn Dũng", "Hoàng Văn Em", "Huỳnh Văn Giang", "Phan Văn Hùng", "Vũ Văn Khánh", "Đỗ Văn Long", "Ngô Văn Minh", "Đặng Văn Nam", "Võ Văn Quốc", "Bùi Văn Sơn", "Đinh Văn Tùng", "Lý Văn Út"]
NU = ["Nguyễn Thị Lan", "Trần Thị Mai", "Lê Thị Hoa", "Phạm Thị Hạnh", "Hoàng Thị Hằng", "Huỳnh Thị Huệ", "Phan Thị Khánh", "Vũ Thị Lam", "Đỗ Thị My", "Ngô Thị Nga", "Đặng Thị Oanh", "Võ Thị Phương", "Bùi Thị Quỳnh", "Đinh Thị Rạng", "Lý Thị Sương"]

def get_random_name():
    gioitinh = random.choice(['nam', 'nu'])
    if gioitinh == 'nam':
        return random.choice(NAM)
    else:
        return random.choice(NU)

def thanhngang(so):
    print(f'{xanh}{"="*so}{RESET}')

def banner():
    os.system('cls' if os.name=='nt' else 'clear')
    
    # Logo ASCII Art màu Xanh dương
    logo_text = f"""
{BLUE}███╗░░██╗██╗░░░██╗░█████╗░██╗  ███████╗░█████╗░░█████╗░███████╗██████╗░░█████╗░░█████╗░██╗░░██╗
{BLUE}████╗░██║██║░░░██║██╔══██╗██║  ██╔════╝██╔══██╗██╔══██╗██╔════╝██╔══██╗██╔══██╗██╔══██╗██║░██╔╝
{BLUE}██╔██╗██║██║░░░██║██║░░██║██║  █████╗░░███████║██║░░╚═╝█████╗░░██████╦╝██║░░██║██║░░██║█████═╝░
...
{BLUE}╚═╝░░╚══╝░╚═════╝░░╚════╝░╚═╝  ╚═╝░░░░░╚═╝░░╚═╝░╚════╝░╚══════╝╚═════╝░░╚════╝░░╚════╝░╚═╝░░╚═╝
{CYAN}                 ╔════════════════════════════════════════╗
{CYAN}                 ║  {luc}Tool Nuôi Acc FB (V1.0.0){CYAN}             ║
{CYAN}                 ║  {xanh}Tool Nuôi facebook{CYAN}                    ║
{CYAN}                 ╚════════════════════════════════════════╝
{RESET}"""
...
def Delay(value):
    while not(value <= 1):
        value -= 0.02
        print(f'''{trang}[{BLUE}SYSTEM{trang}] [{CYAN}DELAY{trang}] [{luc}{str(value)[0:5]}s{trang}] [{YELLOW}PROCESSING...{trang}]''', ' ', end = '\r')
        sleep(0.02)
    print(f'''{trang}[{BLUE}SYSTEM{trang}] [{CYAN}DELAY{trang}] [{luc}0.00s{trang}] [{GREEN}DONE !{trang}]       ''', ' ', end = '\r')
    sleep(0.05)
    print()
...
class Facebook:
    ...
    def follow(self, id):
        try:
            data = {
                'av': self.id,'fb_dtsg': self.fb_dtsg,'jazoest': self.jazoest,'fb_api_caller_class': 'RelayModern','fb_api_req_friendly_name': 'CometUserFollowMutation',
    ...
    def group(self, id):
        try:
            data = {
                'av':self.id,'fb_dtsg':self.fb_dtsg,'jazoest':self.jazoest,'fb_api_caller_class':'RelayModern','fb_api_req_friendly_name':'GroupCometJoinForumMutation',
                ...
                'server_timestamps':'true','doc_id':'5853134681430324','fb_api_analytics_tags':'["qpl_active_flow_ids=431626709"]',
            }
...
def addcookie():
    ...
    while True:
        i += 1
        cookie = input(f'{thanh}{trang}Nhập Cookie Facebook Số {cyan}{i}{trang}: {vang}')
        if cookie == '' and i != 1:
            break
        fb = Facebook(cookie)
        info = fb.info()
        if 'success' in info:
            name = info['name']
            print(f'{thanh}{luc}Tên Facebook: {cyan}{name}')
            thanhngang(65)
...
if os.path.exists(f'cookiefb-add.json') == False:
    ...
    print(f'{thanh}{luc}Nhập {xanh}[{cyan}1{xanh}] {luc}Sử Dụng Cookie Facebook Đã Lưu')
    print(f'{thanh}{luc}Nhập {xanh}[{cyan}2{xanh}] {luc}Nhập Cookie Facebook Mới')
    thanhngang(65)
    chon = input(f'{thanh}{trang}Lựa chọn{trang}: {vang}')
    thanhngang(65)
    while True:
        if chon == '1':
...
print(f'\n{xanh}--- CẤU HÌNH TÍNH NĂNG ---{RESET}\n')

comment = input(f"{thanh}{luc}Bật comment dạo {do}({cyan}y/n{do}){luc}: {vang}").lower() == 'y'
group = input(f"{thanh}{do}Bật tham gia nhóm dạo {do}({cyan}y/n{do}){do}: {vang}").lower() == 'y'
reaction = input(f"{thanh}{tim}Bật thả cảm xúc dạo {do}({cyan}y/n{do}){tim}: {vang}").lower() == 'y'
follow = input(f"{thanh}{xanh}Bật follow dạo {do}({cyan}y/n{do}){xanh}: {vang}").lower() == 'y'
add_friend = input(f"{thanh}{vang}Bật gửi lời mời kết bạn dạo {do}({cyan}y/n{do}){vang}: {vang}").lower() == 'y'
...
if comment:
    i = 1
    while True:
        cmt = input(f"{thanh}{hong}Nhập nội dung comment số {cyan}{i}{trang}: {vang}").strip() # Màu hồng
        if cmt == "":
            break
        comments.append(cmt)
        i += 1
...
if reaction:
    ...
    # --- MENU CẢM XÚC CẬP NHẬT MÀU SẮC ---
    print(f'\n{xanh}--- CHỌN LOẠI CẢM XÚC ---{RESET}')
    print(f"{thanh}{trang}Nhập {hong}[1]{trang} Để Chạy Tương Tác {hong}Like") # Hồng
    print(f"{thanh}{trang}Nhập {xanh}[2]{trang} Để Chạy Tương Tác {xanh}Love") # Xanh dương
    print(f"{thanh}{trang}Nhập {hong_nhat}[3]{trang} Để Chạy Tương Tác {hong_nhat}Care") # Hồng nhạt
    print(f"{thanh}{trang}Nhập {tim}[4]{trang} Để Chạy Tương Tác {tim}Haha") # Tím
    print(f"{thanh}{trang}Nhập {luc}[5]{trang} Để Chạy Tương Tác {luc}Wow") # Xanh lá
    print(f"{thanh}{trang}Nhập {do}[6]{trang} Để Chạy Tương Tác {do}Sad") # Đỏ
    print(f"{thanh}{trang}Nhập {vang}[7]{trang} Để Chạy Tương Tác {vang}Angry") # Vàng
    print(f"{thanh}{cyan}Có Thể Chọn Nhiều Tương Tác {do}({cyan}VD: 1345...{do})")
    thanhngang(65)
    chon = input(f'{thanh}{luc}Nhập Số Để Chọn Tương Tác{trang}: {vang}').strip()
    ds_camxuc = {
...
# --- CẤU HÌNH DELAY ---
print(f'\n{xanh}--- CẤU HÌNH THỜI GIAN & CHỐNG BLOCK ---{RESET}\n')
...
while True:
    try:
        ...
        if delay >= 0: 
            ...
            print(f'{do}Vui Lòng Nhập Số Dương')
    except:
...
while True:
    try:
        ...
        if JobbBlock >= 1: 
            ...
            print(f'{do}Vui Lòng Nhập Lớn Hơn 0')
    except:
        print(f'{do}Vui Lòng Nhập Số')

while True:
    try:
        DelayBlock = int(input(f'{thanh}{luc}Sau {cyan}{JobbBlock} {luc}Nhiệm Vụ Nghỉ Bao Nhiêu Giây{trang}: {vang}'))
        if DelayBlock >= 1:
            break
        else:
            print(f'{do}Vui Lòng Nhập Lớn Hơn 0')
...
runidfb = input(f'{thanh}{luc}Bạn Có Muốn Ẩn Id Facebook Không? {do}({cyan}y/n{do}){luc}: {vang}')
thanhngang(65)
stt = 0
...
while True:
    ...
    for cookie in listCookie:
        ...
        print(f'\n{luc}► Id Facebook{trang}: {cyan}{idrun}{do} | {luc}Tên Tài Khoản{trang}: {cyan}{namefb}{trang}')
        thanhngang(50)
        ...
        while True:
            try:
                ...
                if nhiemvu == "comment":
                    ...
                    if getpost.get('trangthai') == 'thatbai':
                        ...
                        if cmt.get('trangthai') == 'thanhcong':
                            ...
                            JobSuccess += 1
                            stt += 1
                            print(f'{do}| {cyan}#{stt}{do} | {xanh}{timejob}{do} | {luc}COMMENT{do} | {BLUE}{noidung}{do}')
                        else:
                            JobFail += 1
                ...
                if nhiemvu == "group":
                    ...
                    if timgr.get('trangthai') == 'thatbai':
                        ...
                        if thamgia.get('trangthai') == 'thanhcong':
                            ...
                            JobSuccess += 1
                            stt += 1
                            print(f'{do}| {cyan}#{stt}{do} | {xanh}{timejob}{do} | {luc}JOIN GROUP{do} | {trang}{id}{do} | {cyan}{name}{do}')
                        else:
                            JobFail += 1
                ...
                if nhiemvu == "reaction":
                    ...
                    if getpost.get('trangthai') == 'thatbai':
                        ...
                        if tha.get('trangthai') == 'thanhcong':
                            ...
                            JobSuccess += 1
                            stt += 1
                            print(f'{do}| {cyan}#{stt}{do} | {xanh}{timejob}{do} | {luc}REACTION{do} | {BLUE}{camxuc}{do} | {trang}{idpost}{do}')
                        else:
                            JobFail += 1
                ...
                if nhiemvu == "follow":
                    ...
                    if timbb.get('trangthai') == 'thatbai':
                        ...
                        if fb.follow(id).get('trangthai') == 'thanhcong':
                            ...
                            JobSuccess += 1
                            stt += 1
                            print(f'{do}| {cyan}#{stt}{do} | {xanh}{timejob}{do} | {luc}FOLLOW{do} | {trang}{id}{do} | {cyan}{name}{do}')
                        else:
                            JobFail += 1
                ...
                if nhiemvu == "add_friend":
                    ...
                    if timbb.get('trangthai') == 'thatbai':
                        ...
                        if ketban.get('trangthai') == 'thanhcong':
                            ...
                            JobSuccess += 1
                            stt += 1
                            print(f'{do}| {cyan}#{stt}{do} | {xanh}{timejob}{do} | {luc}ADD FRIEND{do} | {trang}{id}{do} | {cyan}{name}{do}')
                        else:
                            JobFail += 1
                ...
                if JobFail >= 20:
                    check = fb.info()
                    if 'spam' in check:
                        print(f'{do}Tài Khoản {cyan}{namefb} {do}Đã Bị Spam')
                        fb.clickDissMiss()
                    elif '282' in check:
                        print(f'{do}Tài Khoản {cyan}{namefb} {do}Đã Bị Checkpoint282')
                        listCookie.remove(cookie)
                        chuyen = True
                        break
                    elif '956' in check:
                        print(f'{do}Tài Khoản {cyan}{namefb} {do}Đã Bị Checkpoint956')
                        listCookie.remove(cookie)
                        chuyen = True
                        break
                    else:
                        print(f'{do}Tài Khoản {cyan}{namefb} {do}Đã Bị Out Cookie, Đã Xoá Khỏi List')
                        listCookie.remove(cookie)
                        chuyen = True
                        break
